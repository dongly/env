...
def copy_env_scripts(config):
    ...
    (no absolute paths, no injected variables). The user customization file
    is created only when missing: it lives outside the managed repository,
    so upgrades and reinstalls never overwrite it. The same generation
    logic is mirrored in cmds/cmd_upgrade.py (_write_root_activator) for
    the pkgs --upgrade refresh path.
    """
    ...
    if name.endswith('.ps1'):
        ...
        content = (
            "# Generated by the RT-Thread ENV installer. Do not edit.\n"
            '. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/tools/scripts/%s"\n' % name
        )
        encoding = 'utf-8'
...
def check_existing_env(config):
    ...
    if keep:
        ...
        root_config = os.path.join(config.env_root, 'sdk.config')
